...
c = 0
for b in buttons:
    action = lambda x=b: press(x) if x != '=' else cal()
    tk.Button(root, text=b, width=5, height=2, font=("Segoe UI", 14), bg='#3c3c3c', fg='white', bd=0,
              command=action).grid(row=r, column=c, padx=5, pady=5)
    c += 1
    if c > 3:
        c = 0
        r += 1 
tk.Button(root,text="c",command=clear,font=("Segoe UI",14),bg="#ff3b3b",fg="white",bd=0,width=22,height=2) \
